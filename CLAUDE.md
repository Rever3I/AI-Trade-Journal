...
1. **Security first:** NEVER expose API keys client-side. ALL external API calls go through Cloudflare Worker proxy.
2. **Chinese-first:** UI, error messages, and AI outputs default to Chinese. Support English as secondary.
3. **Cost-conscious:** Track token usage per user. Implement rate limits from day 1, not as an afterthought.
4. **Graceful degradation:** If Claude API fails, show cached results or friendly error — never crash.
5. **Prompt versioning:** ALL AI prompts live in `/src/prompts/` as versioned files. NEVER hardcode prompts in business logic.
6. **Test with real data:** Use actual (anonymized) broker exports for testing, not synthetic data.
7. **Manifest V3 only:** No deprecated V2 APIs. Follow latest Chrome Extension best practices.
8. **Incremental delivery:** Each PR should be deployable. No "big bang" merges.
